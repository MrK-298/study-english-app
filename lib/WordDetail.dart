    ...
    if (response.statusCode == 200) {
      ...
      for (var meaning in data[0]['meanings']) {
          ...
          for (var definition in meaning['definitions']) {
            List<String> synonyms = definition['synonyms'] != null ? List<String>.from(definition['synonyms']) : [];
            List<String> antonyms = definition['antonyms'] != null ? List<String>.from(definition['antonyms']) : [];
          ...
          List<String> synonymsAtMeaningLevel = meaning['synonyms'] != null ? List<String>.from(meaning['synonyms']) : [];
          List<String> antonymsAtMeaningLevel = meaning['antonyms'] != null ? List<String>.from(meaning['antonyms']) : [];

          meaningDefinitions.add(DefinitionData(
            definition: '', // Nếu có synonyms và antonyms ở cấp độ cao hơn, bạn có thể để definition là chuỗi trống hoặc điều gì đó phù hợp
            ...
            children: [
              ...
              for (var partOfSpeechData in partsOfSpeech)
                Column(
                  ...
                  children: [
                    ListTile(
                      title: Text('Loại từ: ${partOfSpeechData.partOfSpeech}', style: TextStyle(fontSize: 20)),
                    ),
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            if(definitionData.definition.isNotEmpty)
                              Text('Nghĩa: ${definitionData.definition.isNotEmpty ? definitionData.definition : ''}', style: TextStyle(fontSize: 20,color: Colors.black)),
                            if (definitionData.synonyms.isNotEmpty)
                              Text('\nTừ đồng nghĩa: ${definitionData.synonyms.join('\t,')}',style: TextStyle(fontSize: 20,color: Colors.black)),
                            if (definitionData.antonyms.isNotEmpty)
                              Text('\nTừ trái nghĩa: ${definitionData.antonyms.join('\t,')}',style: TextStyle(fontSize: 20,color: Colors.black)),
                            if (definitionData.example.isNotEmpty)
                              Text('\nVí dụ: ${definitionData.example}', style: TextStyle(fontSize: 20,color: Colors.black)),
                          ],
                        ),
                      ),
                  ],
