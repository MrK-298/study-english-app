...
class _DetailTopicPageState extends State<DetailTopicPage> {
  ...
  Future<void> getAllHomeworks() async {
   ...
   Future<void> getHomeworksByUserId() async {
    final response = await http.get(
      Uri.parse('https://10.0.2.2:7142/api/Homework/GetUserHomeworks?userId=$userId'),
      headers: {
        'Content-Type': 'application/json',
      },
    );
    ...
    if (response.statusCode == 200) {
      ...
      userHomeworks = userHomeworksData.map((data) => Homework.fromJson(data)).toList();
    }
  }
  Future<void> getTopic() async {
    final response = await http.get(
      Uri.parse('https://10.0.2.2:7142/api/Topic/GetTopicWithId?id=${widget.topicId}'),
      headers: {
        'Content-Type': 'application/json',
      },
    );
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (homework.isDone==false) {
                ...
                cardColor = Color.fromARGB(255, 204, 87, 87); // Đỏ nếu score < 5
              } else if (homework.score < 8) {
                cardColor = Color.fromARGB(255, 201, 190, 94); // Vàng nếu 5 <= score < 8
              } else {
                cardColor = Color.fromARGB(255, 121, 210, 142); // Xanh lá nếu score >= 8
              }
